        ...
        async with self.lock:
            ...
            async def emote_cache_helper(ch_emote, emote_cache):
                ...
                try:
                    ...
                    log.debug(
                        f"Checking if {emote_id} (:{emote_name}:) is accessible without cache."
                    )
                    for chk_emote in emote_cache.emojis:
                        if int(emote_id) == chk_emote.id:
                            log.debug(
                                log.debug(
                                    f"{emote_id} (:{emote_name}:) is accessible. Skipping..."
                                )
                            )
                            return
                    ...
                    if emote_animated:
                        ...
                        emote_image = requests.get(
                            f"https://cdn.discordapp.com/emojis/{emote_id}.gif"
                        ).content
                    else:
                        log.debug(f"{emote_id} (:{emote_name}:) is not animated")
                        emote_image = requests.get(
                            f"https://cdn.discordapp.com/emojis/{emote_id}.webp"
                        ).content
                    log.debug(f"Uploading emote {emote_id} (:{emote_name}:)")
                    cached_emote = await emote_cache.create_custom_emoji(
                        name=emote_name, image=emote_image
                    )
                    log.debug(f"{emote_id} (:{emote_name}:) uploaded")
                    return (
                        ch_emote,
                        f'<{"a" if emote_animated else ""}:{cached_emote.name}:{cached_emote.id}>',
                        cached_emote,
                    )
                except discord.Forbidden:
                    log.debug(
                        "Polyphony does not have permission to upload emote cache emoji"
                    )
                    return
            ...
            if emote_cache:
                ...
                try:
                    new_emotes = await asyncio.wait_for(
                        asyncio.gather(*task_list), timeout=3
                    )
                    ...
                    log.debug(
                        "DEBUG WARNING: Emote cached timed out. Disabling for 1 minute."
                    )
                    self.emote_cache_rate_limit_timeout = datetime.now() + timedelta(
                        minutes=1
                    )
